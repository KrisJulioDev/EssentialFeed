...
final class FeedAcceptanceTests: XCTestCase {
    ...
    func test_onLaunch_displaysCachedRemoteFeedWhenCustomerHasNoConnectivity() {
        ...
        XCTAssertEqual(offlineFeed.renderedFeedImageData(at: 0)?.count, makeImageData0().count)
        XCTAssertEqual(offlineFeed.renderedFeedImageData(at: 1)?.count, makeImageData1().count)
        XCTAssertEqual(offlineFeed.renderedFeedImageData(at: 2)?.count, makeImageData2().count)
    }
    ...
    private func launch(
        ...
        sut.window = UIWindow(frame: .init(origin: .zero, size: .init(width: 1, height: 1)))
        sut.configureWindow()
