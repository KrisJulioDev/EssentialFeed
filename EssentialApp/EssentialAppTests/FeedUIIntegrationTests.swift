...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoreActions_requestMoreFromLoader() {
        ...
        XCTAssertEqual(loader.loadMoreCallCount, 0, "Expecting no request before until load more action")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expecting load more call request")
        
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expecting no request while loading more")
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected request after loading all pages")
    }
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() {
        ...
        loader.completeFeedLoading(at: 0)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator once load is complete")

        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected loading indicator when loading more")
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertTrue(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator when load more completes")
        
        loader.completeLoadMoreWithError(at: 1)
        XCTAssertFalse(sut.isShowingLoadMoreFeedIndicator, "Expected no loading indicator when load more completes")
    }
    ...
    func test_feedImageView_configuresViewCorrectlyWhenCellBecomingVisibleAgain() {
        ...
        XCTAssertEqual(view0?.renderedImage?.count, imageData.count, "Expected rendered image when image loads successfully after view becomes visible again")
    ...
    func test_feedImageView_shouldNotRenderImageIfCellIsNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        ...
        XCTAssertNil(view?.renderedImage, "Expect renderedImage is nil when cell is reused and not visible")
    }
    ...
    func test_feedImageView_configuresViewCorrectlyWhenTransitioningFromNearVisibleToVisibleWhileStillPreloadingImage() {
        ...
        XCTAssertEqual(view0?.renderedImage?.count, imageData.count, "Expected rendered image after image preloads successfully")
